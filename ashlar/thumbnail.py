...
def calculate_scale(reader, default_scale=0.05):
    positions = reader.metadata.positions - reader.metadata.origin
    full_shape = (positions + reader.metadata.size).max(axis=0)
    mshape = full_shape * default_scale
    # Make thumbnail a minimum of 1000 along the smallest dimension.
    if all(mshape < 1000):
        if all(full_shape > 1000):
            scale = 1000 / max(full_shape)
        else:
            scale = 1
    else:
        scale = default_scale
